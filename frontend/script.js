...
function setup() {
    $("#chat_send").click(sendInput),
        ...
        socket.on("talk", function (a) {
            var b = bonzis[a.guid];
            b.cancel(), b.runSingleEvent([{ type: "text", text: a.text, originalText: a.originalText }]);
        }),
        ...
        socket.on("quote", function (a) {
            var b = bonzis[a.guid];
            b.cancel(), b.quote(a.quotedText, a.quotedUser);
        }),
        socket.on("quoteError", function (a) {
            // Show error when there's no message to quote
            var errorDiv = $("<div class='quote-error' style='position:fixed;top:20px;right:20px;background:#f44336;color:white;padding:10px;border-radius:5px;z-index:9999;'>" +
                a.message +
        ...
        socket.on("dm", function (a) {
            // Show DM in a more elegant way
            var dmDiv = $("<div class='dm-notification' style='position:fixed;top:20px;right:20px;background:#333;color:white;padding:15px;border-radius:5px;z-index:9999;max-width:300px;'>" +
                "<strong>DM from " + a.from + ":</strong><br>" + a.message +
                "<br><button onclick='$(this).parent().remove()' style='margin-top:10px;padding:5px 10px;background:#666;border:none;color:white;border-radius:3px;cursor:pointer;'>Close</button>" +
                "</div>");
            $("body").append(dmDiv);
            
            // Auto-remove after 10 seconds
            setTimeout(function() {
                dmDiv.fadeOut(function() { $(this).remove(); });
            }, 10000);
        }),
        socket.on("dmSent", function (a) {
            // Show confirmation that DM was sent
            var confirmDiv = $("<div class='dm-confirmation' style='position:fixed;top:20px;right:20px;background:#4CAF50;color:white;padding:10px;border-radius:5px;z-index:9999;'>" +
                "DM sent to " + a.to +
                "<br><button onclick='$(this).parent().remove()' style='margin-top:5px;padding:3px 8px;background:#45a049;border:none;color:white;border-radius:3px;cursor:pointer;'>Close</button>" +
                "</div>");
            $("body").append(confirmDiv);
            
            // Auto-remove after 5 seconds
            setTimeout(function() {
                confirmDiv.fadeOut(function() { $(this).remove(); });
            }, 5000);
        }),
        socket.on("typingUpdate", function (a) {
            console.log("typingUpdate received:", a);
            // Update typing indicators for all users
            updateTypingIndicators(a.typingUsers);
        }),
...
function usersUpdate() {
    ...
    for (var i = 0; i < usersAmt; i++) {
        var guid = usersKeys[i];
        var bonzi = bonzis[guid];
        if (bonzi) {
            var isTyping = typingUsers.includes(parseInt(guid));
            bonzi.updateTypingIndicator(isTyping);
        }
    }
}
function sendInput() {
...
function updateTypingIndicators(typingUserGuids) {
    console.log("updateTypingIndicators called with:", typingUserGuids);
    console.log("Current usersKeys:", usersKeys);
    console.log("Current typingUsers:", typingUsers);
    typingUsers = typingUserGuids;
    
    // Update all Bonzi names to show/hide typing indicators
    for (var i = 0; i < usersAmt; i++) {
        var guid = usersKeys[i];
        var bonzi = bonzis[guid];
        if (bonzi) {
            // Use the bonzi's actual GUID instead of the usersKeys GUID
            var isTyping = typingUsers.includes(bonzi.userPublic.guid.toString());
            console.log("User", guid, "bonzi.userPublic.guid:", bonzi.userPublic.guid, "isTyping:", isTyping);
            bonzi.updateTypingIndicator(isTyping);
        }
    }
}
            ...
            _createClass(a, [
                ...
                },
                {
                    ...
                    value: function (a, b) {
                        ...
                        // Only update sprite position if sprite exists
                        if (this.sprite) {
                ...
                },
                {
                    key: "updateAnim",
                    value: function () {
                        // Check if sprite exists before trying to use it
                        if (!this.sprite) {
                            console.log("Sprite not initialized for", this.userPublic.name);
                            this.eventNext();
                            return;
                        }
                ...
                },
                {
                    key: "updateText",
                    value: function () {
                        0 === this.event.timer && (this.$dialog.css("display", "block"), (this.event.timer = 1), this.talk(this.event.cur().text, this.event.cur().say || this.event.cur().originalText, !0)), "none" == this.$dialog.css("display") && this.eventNext();
                    },
                },
                {
                    key: "updateIdle",
                ...
                },
                {
                    ...
                    value: function (a, b, c) {
                        ...
                        // Hacky fix: unescape all HTML tags for markdown display
                        a = a.replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&amp;/g, "&");
                        this.$dialogCont[c ? "html" : "html"](a)[e ? "addClass" : "removeClass"]("bubble_greentext").css("display", "block"),
                            this.stopSpeaking(),
                            (this.goingToSpeak = !0),
                            // AGGRESSIVE HTML STRIPPING - REMOVE ALL TAGS WITH EXTREME PREJUDICE (TTS ONLY)
                            speak.play(
                                (this.originalText || b || a).replace(/<[^>]*>/g, "").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&amp;/g, "&").replace(/<[^>]*>/g, ""),
                ...
                },
                {
                    ...
                    value: function () {
                        this.stopSpeaking();
                        if (this.sprite && BonziHandler.stage.contains(this.sprite)) {
                            BonziHandler.stage.removeChild(this.sprite);
                        }
                        (this.run = !1), this.$element.remove();
                    },
                },
                {
                    key: "updateName",
                    value: function () {
                        console.log("updateName called for", this.userPublic.name, "isTyping:", this.isTyping);
                        if (this.isTyping) {
                            this.$nametag.html(this.userPublic.name + " (typing)");
                        } else {
                            this.$nametag.html(this.userPublic.name);
                        }
                        console.log("Name updated to:", this.$nametag.html());
                    },
                },
                {
                    key: "updateTypingIndicator",
                    value: function (isTyping) {
                        console.log("updateTypingIndicator called for", this.userPublic.name, "isTyping:", isTyping);
                ...
                },
                {
                    ...
                    value: function (a) {
                        if (!this.mute) {
                            this.$dialogCont.html(
                                "\n\t\t\t\t\t<img src='" + a + "' style='width:173px;height:173px;object-fit:fill;' alt='Image'>\n\t\t\t\t"
                            ),
                                this.$dialog.show();
                        }
                    },
                },
                {
                    ...
                    value: function (a) {
                        if (!this.mute) {
                            this.$dialogCont.html(
                                "\n\t\t\t\t\t<video src='" + a + "' style='width:173px;height:173px;object-fit:cover;' controls autoplay></video>\n\t\t\t\t"
                            ),
                                this.$dialog.show();
                        }
                    },
                ...
                {
                    key: "quote",
                    value: function (quotedText, quotedUser) {
                        if (!this.mute) {
                            // Clean the quoted text for TTS
                            var cleanText = quotedText.replace(/<[^>]*>/g, "").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&amp;/g, "&").replace(/<[^>]*>/g, "");
                            var cleanUser = quotedUser.replace(/<[^>]*>/g, "");
                            
                            this.runSingleEvent([
                                { type: "text", text: cleanUser + " said: \"" + quotedText + "\"", say: cleanUser + " said " + cleanText },
                                { type: "idle" }
                            ]);
                        }
                    },
                ...
                },
                {
                    key: "asshole",
                    value: function (a) {
                        // AGGRESSIVE HTML STRIPPING - REMOVE ALL TAGS WITH EXTREME PREJUDICE
                        var cleanTarget = a.replace(/<[^>]*>/g, "").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&amp;/g, "&").replace(/<[^>]*>/g, "");
                ...
                },
                {
                    ...
                    value: function (a) {
                        ...
                        var cleanTarget = a.replace(/<[^>]*>/g, "").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&amp;/g, "&").replace(/<[^>]*>/g, "");
                        this.runSingleEvent([
                            { type: "text", text: "*notices " + a + "'s BonziBulge™*", say: "notices " + cleanTarget + "s bonzibulge" },
                ...
                },
                {
                    key: "updateSprite",
                    value: function (a) {
                        // Check if spritesheets are loaded
                        if (!BonziHandler.spriteSheets || !BonziHandler.spriteSheets[this.color]) {
                            console.log("Spritesheet not loaded for color:", this.color);
                        ...
                        // Check if sprite exists before trying to remove it
                        if (this.sprite && b.contains(this.sprite)) {
                ...
                (this.bonzisCheck = function () {
                    for (var a = 0; a < usersAmt; a++) {
                        ...
                        if (b in bonzis) {
                            ...
                            // Check if spritesheets are loaded before creating new Bonzi
                            if (this.spriteSheets && Object.keys(this.spriteSheets).length > 0) {
                                bonzis[b] = new Bonzi(b, usersPublic[b]);
                            } else {
                                console.log("Spritesheets not loaded yet, delaying Bonzi creation for user", b);
                                // Retry after a short delay
                                setTimeout(() => {
                                    this.bonzisCheck();
                                }, 100);
...
var usersAmt = 0,
    usersKeys = [];
var typingUsers = []; // Track who is currently typing
var typingTimer = null; // Timer for typing stop
$(window).load(function () {
